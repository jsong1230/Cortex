...
// F-24: 기술 트렌드 3줄 요약, 세렌디피티 효과, 주간 포커스 코멘트

import type { SerendipityReport } from '@/lib/weekly-summary';
...
export interface WeeklyDigestData {
  ...
  /** AI 한줄 코멘트 (F-16) */
  aiComment?: string;
  /** F-24 AC1: 이번 주 기술 트렌드 3줄 요약 */
  techTrends?: string;
  /** F-24 AC2: 세렌디피티 효과 측정 리포트 */
  serendipityEffect?: SerendipityReport;
  /** F-24 AC3: AI 주간 포커스 코멘트 */
  focusComment?: string;
}
...
export function formatWeeklyDigest(data: WeeklyDigestData): string {
  ...
  // ─ F-24 AC1: 이번 주 기술 트렌드 ─────────────────────────────────────────
  if (data.techTrends && data.techTrends.trim().length > 0) {
    lines.push('');
    lines.push('📊 <b>이번 주 기술 트렌드</b>');
    lines.push(data.techTrends.trim());
  }

  // ─ F-24 AC2: 세렌디피티 효과 ─────────────────────────────────────────────
  if (data.serendipityEffect && data.serendipityEffect.totalSerendipityItems > 0) {
    const effect = data.serendipityEffect;
    lines.push('');
    lines.push('🎲 <b>세렌디피티 효과</b>');
    lines.push(
      `발견 아이템: ${effect.totalSerendipityItems}개 | 긍정 반응: ${effect.positiveReactions}개 | 효과: ${effect.effectScore}%`,
    );
    if (effect.discoveredTopics.length > 0) {
      lines.push(`새로 발견한 관심사: ${effect.discoveredTopics.join(', ')}`);
    }
  }

  // ─ F-24 AC3: 주간 포커스 코멘트 ──────────────────────────────────────────
  if (data.focusComment && data.focusComment.trim().length > 0) {
    lines.push('');
    lines.push('🎯 <b>주간 포커스</b>');
    lines.push(data.focusComment.trim());
  }
