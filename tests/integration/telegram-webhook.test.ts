...
describe('POST /api/telegram/webhook — callback_query 처리 (I-07-04)', () => {
  ...
  it('I-07-04-1: like:uuid 콜백 처리 후 200을 반환하고 좋아요 반응을 INSERT한다 (F-11 중복 방지)', async () => {
    const { POST } = await import('@/app/api/telegram/webhook/route');
  ...
  it('I-07-04-2: dislike:uuid 콜백 처리 후 싫어요 반응을 INSERT한다 (F-11 중복 방지)', async () => {
    const { POST } = await import('@/app/api/telegram/webhook/route');
  ...
  it('I-07-04-3: save:uuid 콜백 처리 후 저장 반응을 INSERT한다 (F-11 중복 방지)', async () => {
    const { POST } = await import('@/app/api/telegram/webhook/route');
